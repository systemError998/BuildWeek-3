...
export default function ImageApi({img}) {
    ...
    const fetchImages = async () => {
        const query = `${img}`; //da cambiare con la variabile TEXT del POST linkedin
        ...
        try {
            const response = await axios.get(url, {
                headers: {
                    Authorization: 'Client-ID ngik-lTsXKz188yAbtdkR9NQZIcvDyx252A6CXrMnUM'
                }
            });
  ...
  return (
    <>
    {image && <Card.Img variant="top" className='rounded-0' src={image} />}
    </>
  )
} */
